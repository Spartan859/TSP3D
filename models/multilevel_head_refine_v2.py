...
class SimpleRefineHead(nn.Module):
    def __init__(self, in_ch=32, grid_size=6, hidden=256):
        super().__init__()
        self.grid_size = grid_size
        self.shared_fc = nn.Sequential(
            nn.Linear(grid_size * grid_size * grid_size * in_ch, hidden),
    ...
    def forward(self, pooled_feats):
        ...
        # pdb.set_trace()
        x = pooled_feats.permute(0, 4, 1, 2, 3).contiguous()  # (R, C, g, g, g)
...
class DenseRefineHead(nn.Module):
    def __init__(self, in_ch=32, grid_size=6, hidden=256):
        super().__init__()
        self.grid_size = grid_size
        input_dim = in_ch * grid_size * grid_size * grid_size
        ...
        self.reg_fc = nn.Linear(hidden, 6)
        self.cls_fc = nn.Linear(hidden, 1)
    ...
    def forward(self, pooled_feats):
        """
        pooled_feats: (R, gx, gy, gz, C)
        """
        x = pooled_feats.reshape(pooled_feats.shape[0], -1)
        x = self.shared_fc(x)
        reg = self.reg_fc(x)
        cls = self.cls_fc(x)
        return reg, cls
...
class TSPHead(nn.Module):
        ...
        if getattr(self, "use_refine", False) and feats.features.shape[0] > 0:
            ...
            if getattr(self, "use_refine", False):
                ...
                if R > 0:
                    ...
                    # refine_bbox_delta = refine_bbox_delta*0.
    ...
    def _get_bboxes_single(self, bbox_preds, cls_preds, points, img_meta):
        scores = torch.cat(cls_preds).sigmoid()   # (N, 1)
        bbox_preds = torch.cat(bbox_preds)
        points = torch.cat(points)
        ...
        if len(scores) > nms_pre > 0:
            _, ids = max_scores.topk(nms_pre)
            bbox_preds = bbox_preds[ids]
            scores = scores[ids]
            points = points[ids]

        boxes = self._bbox_pred_to_bbox(points, bbox_preds)   # (N, 6)

        # 单类 3DVG，也按多类接口走 NMS
        nms_bboxes, nms_scores, nms_labels = self._nms(boxes, scores, img_meta)

        # 再保留 top-k 候选 ROI
        roi_topk = self.test_cfg.get('roi_topk', 8)
        if len(nms_scores) > roi_topk:
            order = torch.argsort(nms_scores, descending=True)[:roi_topk]
            nms_bboxes = nms_bboxes[order]
            nms_scores = nms_scores[order]
            nms_labels = nms_labels[order]

        return nms_bboxes, nms_scores.unsqueeze(1), nms_labels
    ...
    def forward_test(self, x_all, text_feats, text_attention_mask, targets, inverse_mapping, img_metas, pc=None, gt_bboxes=None):
        ...
        if self.use_seg:
            ...
            best_roi_indices = []   # 记录每个 scene 最终选中的 roi idx（给分割结果用）
            ...
            if getattr(self, "use_refine", False) and refine_delta is not None and refine_delta.numel() > 0:
                ...
                for i, (box3d, score_mat, label) in enumerate(results):
                    ...
                    if roi_count == 0:
                        ...
                        best_roi_indices.append(-1)
                        continue

                    cur_rois = rois[i][:, :6].to(refine_delta.device)   # (K, 6)
                    cur_delta = refine_delta[roi_offset: roi_offset + roi_count]   # (K, 6)

                    # decode all refined boxes
                    cur_refined_boxes = self.decode_roi_residual(cur_rois, cur_delta)   # (K, 6)
                    # pdb.set_trace()
                    if refine_score is None:
                        # 没有 refine score 时退化成用原始检测分数选
                        cur_scores = score_mat.squeeze(-1)
                    else:
                        cur_scores = refine_score[roi_offset: roi_offset + roi_count].sigmoid().squeeze(-1)  # (K,)
                        # cur_scores = cur_scores + score_mat.squeeze(-1)

                    best_local_idx = torch.argmax(cur_scores)
                    best_roi_indices.append(best_local_idx.item())

                    best_box = cur_refined_boxes[best_local_idx:best_local_idx + 1]
                    best_score = cur_scores[best_local_idx:best_local_idx + 1].unsqueeze(1)
                    best_label = label[best_local_idx:best_local_idx + 1] if len(label) == roi_count else label[:1]

                    best_box3d = img_metas[i]['box_type_3d'](
                        best_box,
                        box_dim=6,
                        with_yaw=False,
                        origin=(.5, .5, .5)
                    ...
                    new_results.append((best_box3d, best_score, best_label))
                    roi_offset += roi_count

                results = new_results
            else:
                # 没有 refine 时，直接用检测分数最高的 roi
                best_roi_indices = []
                new_results = []
                for i, (box3d, score_mat, label) in enumerate(results):
                    if len(score_mat) == 0:
                    ...
                    det_scores = score_mat.squeeze(-1)
                    best_local_idx = torch.argmax(det_scores)
                    best_roi_indices.append(best_local_idx.item())

                    best_box = box3d[best_local_idx:best_local_idx + 1]
                    best_score = score_mat[best_local_idx:best_local_idx + 1]
                    best_label = label[best_local_idx:best_local_idx + 1]

                    new_results.append((best_box, best_score, best_label))

                results = new_results

            # pdb.set_trace()
            seg_masks = self._get_instances(
                seg_preds[:, 0],
                idxs[:, 0],
                v2r,
                r2scene,
                ...
                inverse_mapping,
                img_metas,
                best_roi_indices=best_roi_indices
            )
            # pdb.set_trace()
    ...
    def _get_instances(self, cls_preds, idxs, v2r, r2scene, scores, labels, inverse_mapping, img_metas, best_roi_indices=None):
        v2scene = r2scene[v2r]
        results = []
        ...
        for i in range(len(img_metas)):
            ...
            # seg_mask: (num_rois, num_points)
            if seg_mask.ndim == 1:
                final_mask = seg_mask
            else:
                if best_roi_indices is not None and best_roi_indices[i] >= 0 and best_roi_indices[i] < seg_mask.shape[0]:
                    final_mask = seg_mask[best_roi_indices[i]]
                else:
                    final_mask = seg_mask[0]

            results.append(final_mask.bool())
