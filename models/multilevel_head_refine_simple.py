...
class SimpleRefineHead(nn.Module):
    def __init__(self, in_ch=32, hidden=128):
        super().__init__()
...
def roi_avg_pool(feats: ME.SparseTensor, rois_list):
    """
    feats.coordinates: (N,4) [roi_idx, x,y,z]
    feats.features: (N,C)
    ...
    for i in range(R):
        ...
        if mask.sum() == 0:
            roi_feat = torch.zeros(C, device=features.device)
        else:
            roi_feat = features[mask].mean(0)

        roi_feats.append(roi_feat)

    roi_feats = torch.stack(roi_feats, dim=0)
...
class TSPHead(nn.Module):
    ...
    def _init_layers(self, in_channels, out_channels, n_reg_outs, n_classes):
        ...
        if self.use_seg:
            ...
            self.refine_head = SimpleRefineHead(in_ch=32)
    ...
    def _loss_refine(self, rois_list, refine_delta, refine_score_logit, gt_bboxes, img_metas):
        ...
        roi_centers = rois_all[:, :3]
        roi_sizes = rois_all[:, 3:6].clamp(min=1e-6)

        pred_center = roi_centers + refine_delta[:, :3] 
        pred_size = roi_sizes * torch.exp(refine_delta[:, 3:6])
        pred_boxes = torch.cat([pred_center, pred_size], dim=1)
        ...
        if getattr(self, "use_refine", False) and feats.features.shape[0] > 0:
            ...
            if getattr(self, "use_refine", False):
                ...
                if R > 0:
                    ...
                    roi_feats = roi_avg_pool(feats, rois)

                    refine_bbox_delta, refine_score_logit = self.refine_head(roi_feats)
        ...
        if self.use_seg:
            ...
            if getattr(self, "use_refine", False) and refine_delta is not None and refine_delta.numel() > 0:
                ...
                for i, (box3d, score_mat, label) in enumerate(results):
                    ...
                    # roi_center = roi[:3]
                    # roi_size = roi[3:6].clamp(min=1e-6)
                    # delta = refine_delta[roi_offset]
