...
class TSPHead(nn.Module):
    ...
    def _init_layers(self, in_channels, out_channels, n_reg_outs, n_classes):
        ...
        if self.use_seg:
            ...
            self.refine_bbox_loss_weight = 100.0
    ...
    def _loss_single(self,
        ...
        if len(gt_labels) > 0:
            cls_targets_com = torch.where(pos_mask_com, gt_labels[assigned_ids_com], n_classes)
        else:
            cls_targets_com = gt_labels.new_full((len(pos_mask_com),), n_classes)

        com_loss = self.com_loss(com_pred, cls_targets_com)
        ...
        if pos_mask.sum() > 0:
            ...
            # 给 seg/refine 用：原 detection proposal 对应的 score 和 gt index
            proposal_scores = cls_preds[pos_mask]
            proposal_gt_inds = assigned_ids[pos_mask]
        else:
            bbox_loss = None
            box_dim = 6 if bbox_preds.shape[1] == 6 else 7
            pos_bbox_preds_format = bbox_preds.new_zeros((0, box_dim))
            proposal_scores = cls_preds.new_zeros((0, cls_preds.shape[1]))
            proposal_gt_inds = assigned_ids.new_zeros((0,), dtype=torch.long)

        # 额外采样 ROI：只给 seg/refine 用，不参与 detection loss
        extra_boxes, extra_scores, extra_gt_inds = self._sample_extra_rois_for_refine(
            points=points,
    ...
    def _loss(self, bbox_preds, cls_preds, points, gt_bboxes, gt_labels, img_metas, 
        ...
        for i in range(len(img_metas)):
            (bbox_loss, cls_loss, pos_mask, com_loss, pos_mask_com,
             selected_bbox, selected_score, selected_gt_inds,
             extra_bbox, extra_score, extra_gt_inds) = self._loss_single(
            ...
            # 原 detection proposal + 额外采样 ROI
            roi_bbox = torch.cat([selected_bbox.detach(), extra_bbox], dim=0)
            roi_score = torch.cat([selected_score.detach(), extra_score], dim=0)
            roi_gt_inds = torch.cat([selected_gt_inds, extra_gt_inds], dim=0)

            if len(roi_bbox) > self.max_seg_bbox:
                indices = torch.randperm(roi_bbox.shape[0], device=roi_bbox.device)[:self.max_seg_bbox]
                roi_bbox = roi_bbox[indices]
                roi_score = roi_score[indices]
                roi_gt_inds = roi_gt_inds[indices]

            selected_bboxes.append(roi_bbox)
            selected_scores.append(roi_score)
            selected_labels.append(roi_gt_inds)   # 这里其实传的是 gt index
    ...
    @torch.no_grad()
    def _sample_extra_rois_for_refine(self,
                                      points,
                                      bbox_preds,
                                      cls_preds,
                                      assigned_ids,
                                      gt_bboxes):
        """
        从未被分配为 detection proposal 的点中，额外随机采样一批 ROI。
        条件：
          1) assigned_ids < 0，即没有被分配为 detection proposal
          2) 到某个 gt 中心的距离 < min(w, h, l)

        这些 ROI 仅用于 seg/refine，不用于 detection loss。
        """
        device = points.device

        if points.numel() == 0 or len(gt_bboxes) == 0:
            # pdb.set_trace()
            return (
                points.new_zeros((0, 6)),
                cls_preds.new_zeros((0, cls_preds.shape[1])),
                assigned_ids.new_zeros((0,), dtype=torch.long)
            )

        # 预测框（axis-aligned 6维）
        pred_boxes = self._bbox_pred_to_bbox(points, bbox_preds).detach()
        pred_scores = cls_preds.detach()

        gt_boxes = torch.cat(
            (gt_bboxes.gravity_center, gt_bboxes.tensor[:, 3:6]), dim=1
        ).to(device)  # (G, 6)

        # 按你 assigner 的规则，单目标/多目标分别取不同阈值
        if len(gt_bboxes) > 1:
            extra_k = max(1, self.assigner.top_pts_threshold_det // 2)
        else:
            extra_k = max(1, self.assigner.top_pts_threshold // 2)

        # 只能从“没被分配为 detection proposal”的点里采
        available_mask = assigned_ids < 0

        extra_inds = []
        extra_gt_inds = []

        for gt_id in range(len(gt_boxes)):
            gt_center = gt_boxes[gt_id, :3]           # (3,)
            gt_size = gt_boxes[gt_id, 3:6]           # (3,)
            radius = gt_size.min()                   # min(w,h,l)

            dists = torch.norm(points - gt_center[None, :], dim=1)
            cand_mask = available_mask & (dists < radius)
            cand_inds = torch.nonzero(cand_mask, as_tuple=False).squeeze(1)

            if cand_inds.numel() == 0:
                continue

            if cand_inds.numel() > extra_k:
                rand_idx = torch.randperm(cand_inds.numel(), device=device)[:extra_k]
                cand_inds = cand_inds[rand_idx]

            extra_inds.append(cand_inds)
            extra_gt_inds.append(
                torch.full((cand_inds.numel(),), gt_id, dtype=torch.long, device=device)
            )
            # pdb.set_trace()
            # 防止同一个点被多个 gt 重复采到
            available_mask[cand_inds] = False

        if len(extra_inds) == 0:
            return (
                pred_boxes.new_zeros((0, pred_boxes.shape[1])),
                pred_scores.new_zeros((0, pred_scores.shape[1])),
                assigned_ids.new_zeros((0,), dtype=torch.long)
            )

        extra_inds = torch.cat(extra_inds, dim=0)
        extra_gt_inds = torch.cat(extra_gt_inds, dim=0)

        extra_boxes = pred_boxes[extra_inds]     # (N_extra, 6)
        extra_scores = pred_scores[extra_inds]   # (N_extra, C)
        # pdb.set_trace() 

        return extra_boxes, extra_scores, extra_gt_inds
