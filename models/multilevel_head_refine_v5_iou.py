...
class TSPHead(nn.Module):
    ...
    def _init_layers(self, in_channels, out_channels, n_reg_outs, n_classes):
        ...
        if self.use_seg:
            ...
            self.refine_bbox_loss = AxisAlignedIoULoss2(mode='diou', reduction='none')
            self.refine_score_loss = nn.BCEWithLogitsLoss(reduction='mean')
            
            self.refine_score_loss_weight = 1.0
            self.refine_bbox_loss_weight = 2.0
    ...
    def _loss_refine(self, rois_list, refine_delta, refine_score_logit, gt_bboxes, img_metas):
        ...
        for i in range(len(img_metas)):
            ...
            reg_loss = self.refine_bbox_loss(
                self._bbox_to_loss(cur_pred_boxes),
                self._bbox_to_loss(matched_gt)
            )            
