#!/usr/bin/env bash
# Train SR3D segmentation/refine heads from the EA02 epoch-84 checkpoint.
# Uses the same non-freeze LR config as the ScanRefer EA02+SEG+tps32 unfreeze run.
# Optimizer/scheduler state is not loaded because the source EA02 checkpoint has no seg/refine optimizer groups.
set -euo pipefail
...
CHECKPOINT_PATH="${CHECKPOINT_PATH:-${repo_root}/scripts/experiments/sr3d_fixed3/EA02/sr3d/2026-05-18_01-12-46/ckpt_epoch_84.pth}"
...
if ! bash "${switch_dataset_mode_script}" "${mode}" "${data_root}" sr3d; then
    echo "Error: failed to switch SR3D dataset mode to '${mode}'" >&2
    exit 1
fi
