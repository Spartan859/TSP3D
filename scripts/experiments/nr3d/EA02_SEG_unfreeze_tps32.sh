#!/usr/bin/env bash
# Train NR3D segmentation/refine heads from the EA02 epoch-141 checkpoint.
...
CHECKPOINT_PATH="${CHECKPOINT_PATH:-${repo_root}/scripts/experiments/nr3d/EA02/nr3d/2026-05-18_01-13-49/ckpt_epoch_141.pth}"
...
if ! bash "${switch_dataset_mode_script}" "${mode}" "${data_root}" nr3d; then
    echo "Error: failed to switch NR3D dataset mode to '${mode}'" >&2
    exit 1
fi
