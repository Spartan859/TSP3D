#!/usr/bin/env bash
# Train ScanRefer segmentation/refine heads from the EA02 epoch-294 checkpoint.
# Same as EA02_SEG_unfreeze.sh, but enables --use_seg_external_self_attn.
# Uses the same small non-freeze LR config as SEG_tps_thres_seg_single_multi_32_SEGEA_resume2.
# Optimizer/scheduler state is not loaded because the source EA02 checkpoint has no seg optimizer groups.
set -euo pipefail

TSP3D_ENV="${TSP3D_ENV:-/mnt/share/micromamba/root/envs/TSP3D}"
...
BASE_NPROC_PER_NODE="${BASE_NPROC_PER_NODE:-8}"
BS_PER_GPU="${BS_PER_GPU:-4}"
BASE_BS_PER_GPU="${BASE_BS_PER_GPU:-4}"
BASE_LR="${BASE_LR:-5e-5}"
BASE_KEEP_TRANS_LR="${BASE_KEEP_TRANS_LR:-5e-5}"
BASE_TEXT_ENCODER_LR="${BASE_TEXT_ENCODER_LR:-1e-6}"
BASE_BOX_SELECT_LR="${BASE_BOX_SELECT_LR:-4e-5}"
BASE_SEG_LR="${BASE_SEG_LR:-1e-4}"
MAX_EPOCH="${MAX_EPOCH:-400}"
LR_DECAY_EPOCHS="${LR_DECAY_EPOCHS:-30}"
...
CHECKPOINT_PATH="${CHECKPOINT_PATH:-${repo_root}/scripts/experiments/TF32/EA02_num_sps_com_2400/scanrefer/2026-04-18_17-51-17/ckpt_epoch_294.pth}"
...
switch_dataset_mode_script="${repo_root}/scripts/switch_dataset_mode.sh"
...
if ! bash "${switch_dataset_mode_script}" "${mode}" "${data_root}" scanrefer; then
    echo "Error: failed to switch ScanRefer dataset mode to '${mode}'" >&2
    exit 1
fi
